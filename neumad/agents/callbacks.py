...
from cogrip.base_env import CoGripEnv, EPISODE_OUTCOME_SUCCESS
from neumad.agents import utils
from neumad.envs.take_env import trans_obs
...
class RecordVideoCallback(BaseCallback):
    ...
    def log_video(self):
        ...
        if isinstance(env, VecEnv):
            env = env.envs[0]
        obs = env.reset()
        utterance, image = trans_obs(env, obs)
        cache_image(gif_images, image, utterance)
        while True:
            action, _states = policy.predict(obs, deterministic=False if self.phase == "train" else True)
            obs, reward, done, info = env.step(action)
            utterance, image = trans_obs(env, obs)
            if done:
                cache_image(gif_images, image, utterance, info[EPISODE_OUTCOME_SUCCESS] == 1)
                break
            else:
                cache_image(gif_images, image, utterance)
        env.reset()
...
class LatestEvalCheckpointCallback(BaseCallback):

    def __init__(self, save_dir, verbose=0):
        """
        :param save_dir: the model is saved as latest_eval_model.zip in save_path
        """
        super(LatestEvalCheckpointCallback, self).__init__(verbose)
    ...
    def _save_path(self):
        return f"{self.save_dir}/latest_eval_model.zip"
